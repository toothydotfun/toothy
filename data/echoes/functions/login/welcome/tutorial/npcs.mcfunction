...
tellraw @a[tag=tutorial.npcs] ["",{"text":"[] ","bold":true,"color":"white"},{"text":"We've written several data packs to implement unique custom NPCs to Swim.","color":"#4ABFBF"}]
...
#tellraw @a[tag=tutorial.npcs] ["",{"text":"[] ","bold":true,"color":"white"},{"text":"Click here to select a skyblock island (you don't have to!).","underlined":true,"color":"#4ABFBF","clickEvent":{"action":"run_command","value":"!skyblock"}}]
tellraw @a[tag=tutorial.npcs] ""
